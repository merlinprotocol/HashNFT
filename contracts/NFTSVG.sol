...
library NFTSVG {
    ...
    function generateSVG(
        ...
        svg = string(
            abi.encodePacked(
                ...
                '#bitcoin { animation: blink 4s infinite; } </style> <g> <circle cx="145" cy="252" r="100" stroke="white" stroke-width="6" fill="none" /><text id="bitcoin" x="100" y="310" font-family="Arial, sans-serif" font-size="160" font-weight="bold" fill="white">&#x20BF</text></g> <g style="transform:translate(29px, 384px)"> <rect width="120px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-family="\'Courier New\', monospace" font-size="12px" fill="white"> <tspan fill="rgba(255,255,255,0.6)">Token ID: </tspan>',
    ...
    function timestampToMDY(uint256 timestamp) public pure returns (string memory) {
        uint256 secondsInDay = 24 * 60 * 60;
        uint256 daysSince1970 = timestamp.div(secondsInDay);
        uint16 year;
        uint8 month;
        uint8 day;
        uint256 dayInYear;
        ...
        year = 1970;
        dayInYear = 365;
        while (daysSince1970 >= dayInYear) {
            daysSince1970 -= dayInYear;
            year++;
            if (isLeapYear(year)) {
                dayInYear = 366;
            } else {
        ...
        // Calculate month
        month = 1;
        dayInYear = getDaysInMonth(month, year);
        while (daysSince1970 >= dayInYear) {
            daysSince1970 -= dayInYear;
            month++;
            dayInYear = getDaysInMonth(month, year);
        ...
        // Calculate day
        day = uint8(daysSince1970) + 1;

        return string(abi.encodePacked(uintToString(month), "/", uintToString(day), "/", uintToString(year)));
    }
    ...
    function getDaysInMonth(uint8 month, uint16 year) internal pure returns (uint256) {
        if (month == 2) {
    ...
    function uintToString(uint256 number) internal pure returns (string memory) {
        ...
        while (number > 0) {
            value = bytes32(uint256(value) / (2 ** 8));
            value |= bytes32(((number % 10) + 48) * 2 ** 248);
            number /= 10;
        }
