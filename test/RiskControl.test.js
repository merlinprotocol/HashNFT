...
describe('RiskConotrol', function () {
  ...
  beforeEach(async function () {
    [deployer, admin, notAdmin, issuer, test] = await ethers.getSigners()
  ...
  describe('dayNow () uint256', function () {
    ...
    it('success', async function () {
      await gotoStartTime()
      expect(await riskControl.dayNow()).to.equal(0)
    })
  })
  ...
  describe('deliver ()', function () {
    ...
    it('revert already deliver', async function () {
      ...
      await riskControl.connect(admin).setHashNFT(mockHashNFT.address)
      let earnings = await mockEarningsOralce.getRound(day)
  ...
  describe('deliverAllPeriod ()', function () {
    it('success', async function () {
      ...
      let earnings = await mockEarningsOralce.getRound(0)
      let total = earnings.mul(await mockHashNFT.sold()).mul(360)
